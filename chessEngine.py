...
Determining valid moves at current state. It will keep the complete 
...
class GameState:
    def __init__(self):
        ...
        self.pins = []
        self.checks = []
        self.enpassant_possible = ()  # coordinates for the square where en-passant capture is possible (diagonal)
    ...
    def makeMove(self, move):
        """
        Takes a move (as a parameter) and plays it.
        (will not work for castling, pawn promotion and en-passant)
        """
        self.board[move.start_row][move.start_col] = "--"
        self.board[move.end_row][move.end_col] = move.piece_moved
        self.move_log.append(move)  # log  moves so we will be able to undo
        self.white_to_move = not self.white_to_move  # switch players (if there)
        # If king gets moved update's it's location
        if move.piece_moved == "wK":
            self.white_king_location = (move.end_row, move.end_col)
        elif move.piece_moved == "bK":
            self.black_king_location = (move.end_row, move.end_col)

        # promotion of pawn to other pieces
    ...
    def undoMove(self):
        ...
        Undo the last move
        """
        if len(self.move_log) != 0:  # checks that there is a move to undo
            ...
            if move.is_castle_move:
                if move.end_col - move.start_col == 2:  # side of king 
    ...
    def inCheck(self):
        """
        Determine if a current player has a check
        """
        if self.white_to_move:
            ...
            for i in range(1, 8):
                ...
                if 0 <= end_row <= 7 and 0 <= end_col <= 7:
                    ...
                    if end_piece[0] == ally_color and end_piece[1] != "K":
                        ...
                        else:  # 2nd allied piece - no check / pin from this direction
                            break
    ...
    def getPawnMoves(self, row, col, moves):
        """
        Getting all the pawn moves for the pawn located at row, col and adding the moves to the list.
        """
        piece_pinned = False
        pin_direction = ()
                ...
                if (row + move_amount, col - 1) == self.enpassant_possible:
                    ...
                    if king_row == row:
                        ...
                        for i in inside_range:
                            if self.board[row][i] != "--":  # some piece other than en-passant pawn blocks
                                blocking_piece = True
        ...
        if col + 1 <= 7:  # capture to right
            if not piece_pinned or pin_direction == (move_amount, +1):
                ...
                if (row + move_amount, col + 1) == self.enpassant_possible:
                    attacking_piece = blocking_piece = False
                    if king_row == row:
                        if king_col < col:  # king is left to the pawn
    ...
    def getKnightMoves(self, row, col, moves):
        ...
        for move in knight_moves:
            ...
            if 0 <= end_row <= 7 and 0 <= end_col <= 7:
                if not piece_pinned:
                    end_piece = self.board[end_row][end_col]
                    if end_piece[0] != ally_color:  # so its either enemy piece / empty square
                        moves.append(Move((row, col), (end_row, end_col), self.board))

    def getBishopMoves(self, row, col, moves):
        """
        Getting all the bishop moves for the bishop located at row col and adding the moves to the list.
        """
        piece_pinned = False
        pin_direction = ()
        for i in range(len(self.pins) - 1, -1, -1):
    ...
    def getQueenMoves(self, row, col, moves):
        """
        Getting all the queen moves for the queen located at row col and adding the moves to the list.
        """
    ...
    def getKingMoves(self, row, col, moves):
        """
        Getting all the king moves for the king located at row col and adding the moves to the list.
        """
    ...
    def getCastleMoves(self, row, col, moves):
        """
        Generating all valid castle moves for the king at (row, col) and adding them to the list of moves.
        """
        if self.squareUnderAttack(row, col):
            return  # no ability to castle while in check
...
class Move:
    # In CHESS, fields on the board are described by two characters, one of them being number between 1-8 (which is corresponding to rows)
    ...
    # to match the ones used in the real chess game
